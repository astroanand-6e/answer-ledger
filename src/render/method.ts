...
<p>To be explicit, because this pattern has a bad history: <strong>we will never charge to remove anyone, and we will never contact a brand to solicit anything &mdash; not a link, a share, a mention, a reply, or a payment.</strong> There is exactly one reason we will ever write to you: if we have published a dated correction about your product, we tell you, so that you hear it from us rather than from a customer. That notice carries the delist link above, asks you for nothing, and is never followed by a second email. Page titles describe the question, never a company. If you ever see this site do otherwise, it has broken its own rules and you should say so loudly, in public, <a href="https://github.com/${SITE.repo}/issues">in the issue tracker</a>.</p>
